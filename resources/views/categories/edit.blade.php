<x-app-layout>
                    ...
                    <form method="POST" action="{{ route('categories.update', $category) }}">
                        ...
                        <div class="mb-6 p-4 bg-gray-50 rounded-lg">
                            ...
                            <div class="flex items-center gap-3">
                                <span id="preview-icon" class="text-2xl">{{ $category->icon ?? '📝' }}</span>
                                <div>
                                    <p id="preview-name" class="font-semibold">{{ $category->name }}</p>
                                    <span id="preview-badge" class="inline-block px-2 py-1 text-xs rounded" style="background-color: {{ $category->color }}22; color: {{ $category->color }};">
                                        <span id="preview-type">{{ ucfirst($category->type) }}</span> Category
                                    </span>
                                </div>
                                <span id="preview-color" class="w-6 h-6 rounded-full ml-auto" style="background-color: {{ $category->color }};"></span>
                            </div>
                        </div>
    ...
    <script>
        ...
        document.getElementById('type').addEventListener('change', function() {
            ...
            document.getElementById('preview-type').textContent = type || 'Type';
        });
